...
class Label:
    def __init__(self, img, ref, wvl=None, colortab=None,
        ...
        self.imax = self.fig.add_subplot(
            111, autoscale_on=False)
        self.imax.imshow(self.ref)
        self.fig.subplots_adjust(
            left=0.05, right=0.95, bottom=0.0, top=1.0, wspace=0.0)
    ...
    def draw_rois(self):
        ...
        for i, roi in enumerate(self.rois):
            ...
            if roi.selected:
                self.imax.fill(xpts, ypts, fc=roicolor,
                               lw=2, ec='k', ls='dashed')
            else:
    ...
    def label_groups(self):
        ...
        for groupi in np.unique(self.get_groups()):
            grouplab = input('Enter label for group %d: ' % groupi)
            if len(grouplab) > 0:
    ...
    def export_rois(self):
        ...
        for i in np.unique(groupmask)[1:]:
            ...
            print('%d rois in group %d' %
                  (len(np.unique(idmask[groupmask == i])), i))
    ...
    def export_image(self):
        ...
        self.meanfig.canvas.mpl_connect(
            'key_press_event', self.key_press_callback)
    ...
    def key_press_callback(self, event):
        if event.key == 'h':
            ...
            if not self.exported:
                yn = input('ROIs changed, export to file? ')
                if yn.lower() == 'y':
                    self.export_rois()
            sys.exit(1)
